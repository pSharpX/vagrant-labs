...
SERVER_IP=192.168.99.17
WILDFLY_VERSION=20.0.1
WILDFLY_FILE=$WILDFLY_VERSION.Final
WILDFLY_DOWNLOAD_PATH=https://download.jboss.org/wildfly/$WILDFLY_FILE/wildfly-$WILDFLY_FILE.tar.gz
...
wildfly_installation(){
  ...
  wget -nv $WILDFLY_DOWNLOAD_PATH -P /tmp
  # When the download is completed, extract the archive in the /opt directory:
  sudo tar xf /tmp/wildfly-$WILDFLY_FILE.tar.gz -C /opt
  echo "Removing /tmp/wildfly-$WILDFLY_FILE.tar.gz"
  sudo rm /tmp/wildfly-$WILDFLY_FILE.tar.gz

  # We will create a symbolic link wildfly that will point to the Wildfly installation directory:
  sudo ln -s /opt/wildfly-$WILDFLY_FILE /opt/wildfly
  ...
  # sudo echo WILDFLY_CONSOLE_BIND=$SERVER_IP >> /etc/wildfly/wildfly.conf
